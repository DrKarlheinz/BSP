 ...
 * Ken Hughes
 *
 * C library for the MPU6050 gyro
 * Datasheet :
...
void MPU6050::setDebug(bool dbg) { debug = dbg; }
...
{
    // Follow the same initialization sequence as the working test-sensors.cpp

    // Set sample rate divider (0x07 means 8kHz/(1 + 0x07) = 1kHz)
    writeRegister8(0x19, 0x07);

    // Set gyro range (0x08 means +/-500 degs/sec, but let's use 0x00 for +/-250
    // degs/sec)
    writeRegister8(MPU6050_REG_GYRO_CONF, 0x00);

    // Wake up the MPU6050 - set clock source to PLL with X axis gyro reference
    writeRegister8(MPU6050_REG_PWR_MGMT_1, 0x02);

    // Set accelerometer range (0x00 means +/- 2g)
    writeRegister8(MPU6050_REG_ACC_CONF, 0x00);

    debugLog("MPU6050 initialized with working sequence\n");
}

void MPU6050::getAcceleration(int16_t *x, int16_t *y, int16_t *z)
{
    *x = (int16_t)readRegister16(MPU6050_REG_ACC_XOUT_H);
    *y = (int16_t)readRegister16(MPU6050_REG_ACC_YOUT_H);
    *z = (int16_t)readRegister16(MPU6050_REG_ACC_ZOUT_H);
}

void MPU6050::getRotation(int16_t *x, int16_t *y, int16_t *z)
{
    *x = (int16_t)readRegister16(MPU6050_REG_GYRO_XOUT_H);
    *y = (int16_t)readRegister16(MPU6050_REG_GYRO_YOUT_H);
    *z = (int16_t)readRegister16(MPU6050_REG_GYRO_ZOUT_H);
}

float MPU6050::getTemperature()
{
    int16_t temp = (int16_t)readRegister16(MPU6050_REG_TEMP_OUT_H);
...
void MPU6050::readRegister(uint8_t *buffer, uint8_t bytes)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                printf(
                    "Error reading from I2C, expected %i bytes, but got %i\n",
                    bytes, bytesRead);
            }
            else
            {
...
int MPU6050::readRegister16(uint8_t registerAddr)
{
    ...
    writeRegister(buffer, 1); // send the register we are interested in
    readRegister(buffer,
                 2); // get the value from it (HI byte) and the next (LO byte)
    int retVal = (buffer[0] << 8) | buffer[1];
    return retVal;
}
